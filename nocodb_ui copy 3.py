...
def convert_nl_to_query(nl_text, df_columns):
    ...
    Uses OpenAI's GPT to convert a natural language query into a JSON object with two keys:
      ...
      "reasoning": a detailed explanation of the constructed query. This explanation should include:
         - Which columns were selected,
         - How fuzzy matching is applied (using rapidfuzz.fuzz.partial_ratio with a threshold of 70 and case-insensitive matching),
         - Why these columns are deemed relevant,
         - And an explanation of the overall logic.
    """
    ...
    determine which columns are relevant for filtering and produce a JSON object with two keys:
    ...
    "filter_code": A pandas expression that filters the DataFrame using fuzzy matching. For each relevant column,
      ...
      of at least 70 with the specified value (case-insensitive). Combine these conditions using an AND operator.
    
    "reasoning": Provide a detailed explanation of the constructed query. Your explanation should include:
      - Which columns were selected and the value to search for in each,
      - How fuzzy matching is applied (with a threshold of 70 and case-insensitive),
    ...
    {{
      "filter_code": "df[(df['record type'].apply(lambda x: isinstance(x, str) and fuzz.partial_ratio('record type', x.lower()) >= 70)) & (df['Position Start'].apply(lambda x: isinstance(x, str) and fuzz.partial_ratio('24', x.lower()) >= 70))]",
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4",  # or GPT-3.5 if desired
...
def apply_filter_code(df, filter_code):
    ...
    try:
        ...
        st.error(f"Error applying filter code: {e}")
        return df
...
# Apply any renames stored in session state
...
# ---------------------------
# Sidebar: Fuzzy Matching Threshold Slider (Placed at the top)
# ---------------------------
...
if st.session_state["nl_query"].strip():
    ...
    if filter_details and "filter_code" in filter_details:
        # Replace any threshold in the generated code with the user-specified threshold
        modified_filter_code = re.sub(r">=\s*\d+", f">= {threshold}", filter_details["filter_code"])
